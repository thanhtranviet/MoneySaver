...
import com.k22.nhom1.moneysaver.fragment.LoanTabFragment;
 ...
 */
public class EditKhoanChoVayDialog extends DialogFragment {
    public static final String TAG = "EDIT_EXPENSE_FRAGMENT_TAG";
    ...
    EditText txt_transaction_partner;
    EditText txt_transaction_return_date;

    DB4OProvider db;
    Context mContext;
    KhoanChoVay giaodich;
    static Fragment _caller;
    Calendar newCalendar;
    ImageButton btnSelectDate;
    ImageButton btnSelectReturnDate;

    SimpleDateFormat dateFormat;
    boolean isEdit = false;

    public EditKhoanChoVayDialog() {
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        final View view = getActivity().getLayoutInflater().inflate(R.layout.fragment_edit_transaction_extra, null);
        String title = getActivity().getString(R.string.new_transaction_loan);
        ...
        txt_transaction_partner = (EditText) view.findViewById(R.id.txt_transaction_partner);
        txt_transaction_return_date = (EditText) view.findViewById(R.id.txt_transaction_return_date);
        btnSelectDate = (ImageButton) view.findViewById(R.id.btnSelectDate);
        btnSelectReturnDate = (ImageButton) view.findViewById(R.id.btnSelectReturnDate);
        ...
        if (!TextUtils.isEmpty(transactionName)) {
            title = getActivity().getString(R.string.edit_transaction_loan);//"Edit Account Balance";
            giaodich = db.getKhoanChoVay(transactionName);
            ...
            spn_transaction_category.setSelection(categories.indexOf(giaodich.getHangMucChi()));
            spn_transaction_balance.setSelection(balances.indexOf(giaodich.getTaiKhoan()));
            txt_transaction_partner.setText(giaodich.getNguoiVay().toString());
            txt_transaction_return_date.setText(dateFormat.format(giaodich.getNgayDuKienThu()));
            isEdit = true;
        } else {
                ...
                new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int whichButton) {
                        ...
                        try {
                            ...
                            giaodich.setNgayDuKienThu(dateFormat.parse(txt_transaction_return_date.getText().toString()));
                        ...
                        giaodich.setNguoiVay(txt_transaction_partner.getText().toString());
                        ((LoanTabFragment) _caller).onFinishAdd(giaodich);
                    }
                }
        )
    ...
    private List<String> loadCategory() {
        ...
        List<HangMucChi> data = db.findAllHangMucChi();
        for (HangMucChi o : data) {
    ...
    public static EditKhoanChoVayDialog newInstance(String name, Fragment caller) {
        _caller = caller;
        EditKhoanChoVayDialog f = new EditKhoanChoVayDialog();
